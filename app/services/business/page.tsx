...
const activeCat = immigrationMegaMenu.find((c) => c.title === "Business Immigration")!;

const categoryMeta = {
  description:
    "Expert guidance for investor visas, startup permits, and citizenship-by-investment for entrepreneurs and business owners looking to expand globally.",
  accent: "bg-zinc-900",
};
        ...
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6">
            <h1 className="max-w-3xl text-5xl leading-[0.95] font-black tracking-tight text-zinc-900 md:text-7xl">
              {activeCat.title} <br />
              <span className="text-zinc-600 italic">Expand Globally.</span>
            </h1>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {group.items.map((item, iIdx) => (
                      ...
                      >
                        <Link
                          href={item.link}
                          className="group flex items-center justify-between rounded-2xl border border-zinc-100 bg-white p-5 transition-all hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-lg hover:shadow-zinc-500/5"
                        >
                          <span className="text-sm font-bold text-zinc-700 transition-colors group-hover:text-zinc-900">
                            {item.name}
                          </span>
                          <div className="rounded-lg bg-zinc-50 p-2 text-zinc-300 transition-all group-hover:bg-zinc-900 group-hover:text-white">
